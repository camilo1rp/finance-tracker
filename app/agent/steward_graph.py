...
Workflow: fetch unmapped values → list_mappings for the kind (account_id with include_global true returns scoped + global) → list_values or list_transactions for examples → get_transaction when you need raw_type, owner_raw, or override triples → propose ops → always preview before submitting → submit the plan with the preview attached.
